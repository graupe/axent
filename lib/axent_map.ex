defmodule AxentMap do
  ...
  defp maxmin([]), do: []
  defp maxmin([pin = {:^, _pin_meta, [{var_name, _var_meta, context}]} | args]) when is_atom(context), do: [{var_name, pin} | maxmin(args)]
  defp maxmin([var = {var_name, _meta, context} | args]) when is_atom(context), do: [{var_name, var} | maxmin(args)]
